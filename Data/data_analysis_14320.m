...
figure(1);
plot(pixelsum,'*');
title('Pixel Sum for each shot (ODR) IP = 0 mm');
xlabel('Shot Number');
ylabel('Pixel Sum');

saveas(1, '../Figures/Pixelsum_ODR_0', 'png');
...
title('Projected x Intensity (ODR) IP = 0 mm');
xlabel('x (microns)');
ylabel('Intensity');
hold off;

saveas(2, '../Figures/ProjX_ODR_0', 'png');

xaxis1_14320=xaxis1;
x_projection_14320=x_projection(shotx,:);
good_xaxis1_14320=good_xaxis1;
Gauss_xproj_14320=Gauss_xproj;
...
mu=Coeff(2)*sig^2;
A=exp(Coeff(3)+mu^2/(2*sig^2));
mu_nimgs_14320=[mu_nimgs_14320,mu];
sig_nimgs_14320=[sig_nimgs_14320,sig];
sig_total=sig_total+sig;

Gauss_xproj=A*exp(-(good_xaxis1-mu).^2/(2*sig^2));
end

sig_14320=sig_total/n_imgs;
...
title('Projected y Intensity (ODR) IP = 0 mm');
xlabel('y (microns)');
ylabel('Intensity');
hold off;

saveas(3, '../Figures/ProjY_ODR_0', 'png');

figure(4);
imagesc(x_projection);
title('Waterfall Projected x Intensity (ODR) IP = 0 mm');
xlabel('Pixel');
ylabel('Shot Number');

saveas(4, '../Figures/ProjX_wfall_ODR_0', 'png');

figure(5);
imagesc(y_projection);
title('Waterfall Projected y Intensity (ODR) IP = 0 mm');
xlabel('Pixel');
ylabel('Shot Number');

saveas(5, '../Figures/ProjY_wfall_ODR_0', 'png');
...
figure(6);
plot(pixelsum_otr,'*');
title('Pixel Sum for each shot (OTR) IP = 0 mm');
xlabel('Shot Number');
ylabel('Pixel Sum');

saveas(6, '../Figures/PixelSum_OTR_0', 'png');
...
title('Projected x Intensity (OTR) IP = 0 mm');
xlabel('x (microns)');
ylabel('Intensity');
hold off;

saveas(7, '../Figures/ProjX_OTR_0', 'png');
...
title('Projected y Intensity (OTR) IP = 0 mm');
xlabel('y (microns)');
ylabel('Intensity');
hold off;

saveas(8, '../Figures/ProjY_OTR_0', 'png');

figure(9);
imagesc(x_projection_otr);
title('Waterfall Projected x Intensity (OTR) IP = 0 mm');
xlabel('Pixel');
ylabel('Shot Number');

saveas(9, '../Figures/ProjX_wfall_OTR_0', 'png');

figure(10);
imagesc(y_projection_otr);
title('Waterfall Projected y Intensity (OTR) IP = 0 mm');
xlabel('Pixel');
ylabel('Shot Number');

saveas(10, '../Figures/ProjY_wfall_OTR_0', 'png');
...
imagesc(y);
caxis([0 50]);
title('Sample ODR Shot IP = 0 mm');
xlabel('Pixel');
ylabel('Pixel');

saveas(11, '../Figures/Sample_ODR_0', 'png');

figure(12);
x_otr=imread([header data.raw.images.USOTR.dat{i}]);
y_otr=x_otr(y_low_otr:y_high_otr,x_low_otr:x_high_otr);
imagesc(y_otr);
title('Sample OTR Shot IP = 0 mm');
xlabel('Pixel');
ylabel('Pixel');

saveas(12, '../Figures/Sample_OTR_0', 'png');
...
sig_otr=sqrt(-1/(2*Coeff_otr(1)));
mu_otr=Coeff_otr(2)*sig_otr^2;
mu_nimgs_otr_14320=[mu_nimgs_otr_14320,mu_otr];
A_otr=exp(Coeff_otr(3)+mu_otr^2/(2*sig_otr^2));
sig_nimgs_otr_14320=[sig_nimgs_otr_14320,sig_otr];
sig_total_otr=sig_total_otr+sig_otr;

Gauss_xproj_otr=A_otr*exp(-(good_xaxis1_otr-mu_otr).^2/(2*sig_otr^2));
end

sig_otr_14320=sig_total_otr/n_imgs_otr;

figure(13);
plot(sig_nimgs_14320,'*');
title('Image Width for each shot (ODR) IP = 0 mm');
xlabel('Shot Number');
ylabel('Image Width (microns)');

saveas(13, '../Figures/ImageWidth_ODR_0', 'png');

figure(14);
plot(sig_nimgs_otr_14320,'*');
title('Image Width for each shot (OTR) IP = 0 mm');
xlabel('Shot Number');
ylabel('Image Width (microns)');

saveas(14, '../Figures/ImageWidth_OTR_0', 'png');

figure(15);
plot(sig_nimgs_otr_14320,sig_nimgs_14320,'*');
title('Image Width for each shot (OTR) IP = 0 mm');
xlabel('Image Width OTR (microns)');
ylabel('Image Width ODR (microns)');

saveas(15, '../Figures/ImageWidth_OTRODR_0', 'png');

figure(16);
plot(mu_nimgs_otr_14320,mu_nimgs_14320,'*');
title('Image Width for each shot (OTR) IP = 0 mm');
xlabel('Beam Center X Position OTR (microns)');
ylabel('Beam Center X Position ODR (microns)');

saveas(16, '../Figures/XPosition_OTRODR_0', 'png');
...
muy_nimgs_14320=[muy_nimgs_14320,mu2];
...
muy_nimgs_otr_14320=[muy_nimgs_otr_14320,mu2_otr];
...
figure(17);
plot(muy_nimgs_otr_14320,muy_nimgs_14320,'*');
axis([min(muy_nimgs_otr_14320) max(muy_nimgs_otr_14320) 0 3500]);
title('Image Width for each shot (OTR) IP = 0 mm');
xlabel('Beam Center Y Position OTR (microns)');
ylabel('Beam Center Y Position ODR (microns)');

saveas(17, '../Figures/YPosition_OTRODR_0', 'png');

figure(18);
plot(muy_nimgs_otr_14320,muy_nimgs_14320,'*');
axis([min(muy_nimgs_otr_14320) max(muy_nimgs_otr_14320) 0 500]);
title('Image Width for each shot (OTR) IP = 0 mm');
xlabel('Beam Center Y Position OTR (microns)');
ylabel('Beam Center Y Position ODR (microns)');

saveas(18, '../Figures/YPosition_OTRODR_0_zoom', 'png');